...
Page({
  ...
  buildGuide(mountain, forecast) {
    ...
    const weatherAdvice = forecast.isRainy
      ? '雨中观赏云海条件受限，建议等待雨停后转多云或转晴的清晨。'
      : forecast.probability >= 75
      ? `${forecast.guideLevel}，${(forecast.reasons || []).join('、') || '清晨条件较好'}。`
      : `${forecast.guideLevel}，${(forecast.reasons || []).join('、') || '关键条件不够稳定'}。`;
    return [
      `前往${mountain.city}时，${lead}，核心观测窗口为 ${mountain.window}。${weatherAdvice}`,
      `天气建议：湿度 ${displayMetric(forecast.humidity, '%')}，云量 ${displayMetric(forecast.cloud, '%')}，风速 ${displayMetric(forecast.wind, 'm/s')}。${windAdvice}`,
